...
def inventory_create(
    name: str = typer.Argument(
        ..., help="Inventory name (alphanumeric, hyphens, underscores only)"
    ),
    ...
    include_tags: Optional[str] = typer.Option(
        None,
        "--include-tags",
        help="Include only resources with ALL these tags (Key=Value,Key2=Value2)",
    ),
    exclude_tags: Optional[str] = typer.Option(
        None,
        "--exclude-tags",
        help="Exclude resources with ANY of these tags (Key=Value,Key2=Value2)",
    ),
    profile: Optional[str] = typer.Option(
        None,
        "--profile",
    ...
    try:
        ...
        if storage.exists(name, account_id):
            console.print(
                f"✗ Error: Inventory '{name}' already exists for account {account_id}",
                style="bold red",
            )
            console.print(
        ...
        console.print(
            f"✓ Created inventory '[bold]{name}[/bold]' for account {account_id}",
            style="green",
        )
        console.print()
        ...
        if include_tag_dict or exclude_tag_dict:
            ...
            if exclude_tag_dict:
                ...
                console.print(
                    f"    Exclude Tags: {tag_str} (resources must NOT have ANY)"
                )
        else:
...
def inventory_list(
    ...
    try:
        ...
        if not inventories:
            console.print(
                f"No inventories found for account {account_id}", style="yellow"
            )
        ...
        # Create table
        table = Table(
            title=f"Inventories for Account {account_id}",
            show_header=True,
...
def inventory_show(
    ...
    try:
        from ..aws.credentials import get_account_id
        from ..snapshot.inventory_storage import (
        ...
        try:
            ...
            console.print(
                f"✗ Error: Inventory '{name}' not found for account {account_id}",
                style="bold red",
            )
            console.print(
        ...
        console.print(
            f"Created: {inventory.created_at.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        console.print(
            f"Last Updated: {inventory.last_updated.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        console.print()
        ...
        if inventory.snapshots:
            for snapshot_file in inventory.snapshots:
                active_marker = (
                    " [green](active)[/green]"
                    if snapshot_file == inventory.active_snapshot
                    else ""
                )
...
def inventory_migrate(
    ...
    try:
        ...
        if not snapshot_files:
            ...
            console.print(
                "✓ No legacy snapshots found. Nothing to migrate.", style="green"
            )
        ...
        default_inventory = inventory_storage.get_or_create_default(
            identity["account_id"]
        )
        ...
        for snapshot_file in snapshot_files:
            ...
            snapshot_name = snapshot_filename.replace(".yaml.gz", "").replace(
                ".yaml", ""
            )
        ...
        console.print(
            f"✓ Found {legacy_count} snapshot(s) without inventory assignment",
            style="green",
        )
        if added_count > 0:
            console.print(
                f"✓ Added {added_count} snapshot(s) to 'default' inventory",
                style="green",
            )
...
def inventory_delete(
    name: str = typer.Argument(
        ..., help="Inventory name to delete", envvar="AWSINV_INVENTORY_ID"
    ),
    ...
    try:
        ...
        try:
            ...
            console.print(
                f"✗ Inventory '{name}' not found for account {identity['account_id']}",
                style="bold red",
            )
            console.print(
                "  Use 'aws-baseline inventory list' to see available inventories",
                style="yellow",
            )
            raise typer.Exit(code=1)
        ...
        if len(all_inventories) == 1:
            ...
            console.print(
                "  At least one inventory must exist per account", style="yellow"
            )
            raise typer.Exit(code=1)
        ...
        if inventory.active_snapshot:
            console.print(
                "\n⚠️  Warning: This inventory has an active snapshot snapshot!",
                style="bold yellow",
            )
            console.print(
                "   Deleting it will prevent cost/delta analysis for this inventory.",
                style="yellow",
            )
        ...
        try:
            deleted_count = storage.delete(
                name, identity["account_id"], delete_snapshots=delete_snapshots
            )
        except Exception as e:
        ...
        if delete_snapshots and deleted_count > 0:
            ...
            console.print(
                f"  {len(inventory.snapshots)} snapshot file(s) preserved", style="cyan"
            )
...
def config_check(
    ...
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Show detailed resource type support"
    ),
):
    ...
    for region in region_list:
        ...
        if verbose and availability.is_enabled:
            ...
            for service, config_types in sorted(COLLECTOR_TO_CONFIG_TYPES.items()):
                supported_types = [
                    t for t in config_types if availability.supports_resource_type(t)
                ]
                if supported_types:
                    method = "[green]Config[/green]"
                    types_str = ", ".join(
                        t.split("::")[-1] for t in supported_types[:3]
                    )
    ...
    enabled_regions = [
        ...
        if detect_config_availability(session, r, profile).is_enabled
    ]
    if enabled_regions:
        console.print(
            f"[green]Config enabled in {len(enabled_regions)}/{len(region_list)} regions[/green]"
        )
        console.print(
            "[dim]Snapshots will use Config for faster collection where available.[/dim]"
        )
    else:
...
def snapshot_create(
    ...
    set_active: bool = typer.Option(
        True, "--set-active/--no-set-active", help="Set as active snapshot"
    ),
    compress: bool = typer.Option(
        False, "--compress", help="Compress snapshot with gzip"
    ),
    before_date: Optional[str] = typer.Option(
        None,
        "--before-date",
        help="Include only resources created before date (YYYY-MM-DD)",
    ),
    after_date: Optional[str] = typer.Option(
        None,
        "--after-date",
        help="Include only resources created on/after date (YYYY-MM-DD)",
    ),
    filter_tags: Optional[str] = typer.Option(
        None, "--filter-tags", help="DEPRECATED: use --include-tags instead"
    ),
    include_tags: Optional[str] = typer.Option(
        None,
        "--include-tags",
    ...
    use_config: bool = typer.Option(
        False,
        "--config",
        help="Use AWS Config for collection when available (default: disabled, use direct API)",
    ),
    config_aggregator: Optional[str] = typer.Option(
        None,
        "--config-aggregator",
        help="AWS Config Aggregator name for multi-account collection",
    ),
    verbose: bool = typer.Option(
    ...
    try:
        ...
        if inventory:
            ...
            try:
                active_inventory = inventory_storage.get_by_name(
                    inventory, identity["account_id"]
                )
                inventory_name = inventory
                console.print(
                    f"📦 Using inventory: [bold]{inventory}[/bold]", style="cyan"
                )
                ...
                console.print(
                    f"✗ Inventory '{inventory}' not found for account {identity['account_id']}",
                    style="bold red",
                )
                console.print(
                    "  Use 'aws-baseline inventory list' to see available inventories",
            ...
            active_inventory = inventory_storage.get_or_create_default(
                identity["account_id"]
            )
            inventory_name = "default"
        ...
        if inventory:
            ...
            include_tags_dict = (
                active_inventory.include_tags if active_inventory.include_tags else None
            )
            exclude_tags_dict = (
                active_inventory.exclude_tags if active_inventory.exclude_tags else None
            )
        else:
            ...
            if filter_tags:
                console.print(
                    "⚠️  Note: --filter-tags is deprecated, use --include-tags",
                    style="yellow",
                )
                try:
            ...
            if include_tags:
                try:
                    ...
                    console.print(
                        f"✗ Error parsing include-tags: {e}", style="bold red"
                    )
                    raise typer.Exit(code=1)
            ...
            if exclude_tags:
                try:
                    ...
                    console.print(
                        f"✗ Error parsing exclude-tags: {e}", style="bold red"
                    )
                    raise typer.Exit(code=1)
        ...
        if before_date or after_date or include_tags_dict or exclude_tags_dict:
            ...
            if before_date:
                try:
                    ...
                    before_dt = dt.strptime(before_date, "%Y-%m-%d").replace(
                        tzinfo=timezone.utc
                    )
                except ValueError:
                    console.print(
                        "✗ Invalid --before-date format. Use YYYY-MM-DD (UTC)",
                        style="bold red",
                    )
                    raise typer.Exit(code=1)
            ...
            if after_date:
                try:
                    ...
                    after_dt = dt.strptime(after_date, "%Y-%m-%d").replace(
                        tzinfo=timezone.utc
                    )
                except ValueError:
                    console.print(
                        "✗ Invalid --after-date format. Use YYYY-MM-DD (UTC)",
                        style="bold red",
                    )
                    raise typer.Exit(code=1)
        ...
        if use_config:
            ...
            console.print(
                "🔧 AWS Config collection: [bold yellow]disabled[/bold yellow] (using direct API)"
            )
        ...
        if lambda_code_max_size is not None:
            ...
            if lambda_code_max_size == -1:
                console.print(
                    "📦 Lambda code storage: [bold green]unlimited inline[/bold green]"
                )
            elif lambda_code_max_size == 0:
                console.print(
                    "📦 Lambda code storage: [bold cyan]external files only[/bold cyan]"
                )
            else:
                console.print(
        ...
        if created_by_role:
            ...
            console.print(
                f"   Found {len(created_resources)} resource types in CloudTrail"
            )
            console.print(
                f"   Tagged {matched_count}/{len(snapshot.resources)} resources as created by this role"
            )
            console.print(
                "   [dim](Resources older than 90 days won't appear in CloudTrail)[/dim]"
            )
        ...
        if track_creators:
            ...
            event_types = list(EVENT_TO_RESOURCE_TYPE.keys()) + list(
                MULTI_SERVICE_EVENTS.keys()
            )
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"[cyan]Querying {len(event_types)} event types...",
                    total=total_queries,
                )
                ...
                def progress_callback(event_name: str, events_found: int):
                    ...
                    progress.update(
                        task,
                        advance=1,
                        description=f"[cyan]{event_name}: {events_found} events",
                    )
            ...
            console.print(
                f"   Tagged {matched_count}/{len(snapshot.resources)} resources with creator info"
            )
            console.print(
                "   [dim](Resources older than 90 days won't appear in CloudTrail)[/dim]"
            )
        ...
        if set_active:
            console.print(
                "  Marked as active snapshot for this inventory", style="green"
            )
        ...
        if collection_errors:
            console.print(
                f"\n⚠️  Note: {len(collection_errors)} service(s) were unavailable",
                style="yellow",
            )
        ...
        if snapshot.filters_applied:
            ...
            console.print(
                f"  Filtered out: {stats.get('total_collected', 0) - stats.get('final_count', 0)}"
            )
        ...
        if collection_sources:
            ...
            direct_types = [
                t for t, s in collection_sources.items() if s == "direct_api"
            ]

            console.print("\nCollection Method:")
            if config_enabled_regions:
                console.print(
                    f"  AWS Config: [green]Enabled[/green] in {', '.join(config_enabled_regions)}"
                )
                console.print(
                    f"  [green]Config[/green]: {len(config_types)} resource type(s)"
                )
                console.print(
                    f"  [yellow]Direct API[/yellow]: {len(direct_types)} resource type(s)"
                )
            else:
                console.print(
                    "  AWS Config: [yellow]Not enabled[/yellow] (using direct API)"
                )
            ...
            if verbose and (config_types or direct_types):
                console.print()
                method_table = Table(
                    show_header=True, title="Collection Sources by Resource Type"
                )
            ...
            console.print("\nCollection Method:")
            console.print(
                "  All resources collected via Direct API (use --config to enable AWS Config)"
            )
...
def snapshot_show(
    ...
    try:
        ...
        console.print(
            f"Status: {'Active baseline' if snapshot.is_active else 'Inactive'}"
        )
        ...
        if snapshot.service_counts:
            ...
            for service, count in sorted(
                snapshot.service_counts.items(), key=lambda x: x[1], reverse=True
            ):
                percent = (
                    (count / snapshot.resource_count * 100)
                    if snapshot.resource_count > 0
                    else 0
                )
...
def snapshot_enrich_creators(
    name: Optional[str] = typer.Argument(
        None, help="Snapshot name (defaults to active snapshot)"
    ),
    regions: Optional[str] = typer.Option(
    ...
    try:
        ...
        if name:
            ...
            if not active_name:
                console.print(
                    "✗ No active snapshot found. Specify a snapshot name.",
                    style="bold red",
                )
                raise typer.Exit(code=1)
        ...
        for event_name, source_mapping in MULTI_SERVICE_EVENTS.items():
            for res_type in source_mapping.values():
                if (
                    res_type in snapshot_resource_types
                    and event_name not in relevant_event_types
                ):
        ...
        if relevant_event_types:
            console.print(
                f"   Filtering to {len(relevant_event_types)} event types (matching snapshot resources)"
            )
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Querying {event_count} event types...", total=total_queries
            )
...
def snapshot_rename(
    ...
    try:
        ...
        console.print(
            f"✓ Renamed snapshot [bold]{old_name}[/bold] to [bold]{new_name}[/bold]",
            style="green",
        )
...
def snapshot_report(
    snapshot_name: Optional[str] = typer.Argument(
        None, help="Snapshot name (default: active snapshot)"
    ),
    inventory: Optional[str] = typer.Option(
        None, "--inventory", help="Inventory name (required if multiple exist)"
    ),
    profile: Optional[str] = typer.Option(None, "--profile", help="AWS profile name"),
    storage_path: Optional[str] = typer.Option(
        None, "--storage-path", help="Override storage location"
    ),
    ...
    region: Optional[List[str]] = typer.Option(
        None, "--region", help="Filter by region (can specify multiple)"
    ),
    ...
    page_size: int = typer.Option(
        100, "--page-size", help="Resources per page in detailed view (default: 100)"
    ),
    ...
    try:
        ...
        if snapshot_name:
            ...
            console.print(
                f"ℹ Using most recent snapshot from inventory '{inventory}': {target_snapshot_name}",
                style="dim",
            )
        else:
        ...
        try:
            ...
            console.print(
                f"✗ Snapshot '{target_snapshot_name}' not found", style="bold red"
            )
        ...
        if snapshot.resource_count == 0:
            console.print(
                f"⚠️  Warning: Snapshot '{snapshot.name}' contains 0 resources",
                style="yellow",
            )
            ...
            if export:
                try:
                    ...
                    if export_format == "json":
                        ...
                        summary = (
                            reporter.generate_filtered_summary(criteria)
                            if criteria
                            else reporter.generate_summary()
                        )
                        export_path = export_report_json(
                            export, metadata, summary, detailed_resources
                        )
                        console.print(
                ...
                formatter.format_detailed(
                    metadata, detailed_resources, page_size=page_size
                )
        else:
            ...
            if export:
                try:
                    ...
                    if export_format == "json":
                        ...
                        export_path = export_report_json(
                            export, metadata, summary, all_resources
                        )
                        console.print(
...
def snapshot_creators(
    ...
    detailed: bool = typer.Option(
        False, "--detailed", "-d", help="Show individual resources for each creator"
    ),
    export: Optional[str] = typer.Option(
        None,
        "--export",
    ...
    try:
        ...
        resources_without_creator = resource_store.get_resources_without_creator(
        ...
        if export:
            ...
            if ext == ".json":
                ...
                console.print(
                    f"✓ Exported {len(creators_summary)} creators to JSON: {export_path}",
                    style="bold green",
                )
                ...
                with open(export_path, "w", newline="") as f:
                    ...
                    if detailed:
                        ...
                        for creator_info in creators_summary:
                            types_str = ", ".join(
                                f"{k}:{v}"
                                for k, v in creator_info["resource_types"].items()
                            )
                            writer.writerow(
                                [
                                    creator_info["creator"],
                ...
                console.print(
                    f"✓ Exported creators to CSV: {export_path}", style="bold green"
                )
        ...
        console.print(
            f"[dim]Created: {snapshot.created_at.strftime('%Y-%m-%d %H:%M:%S')}[/dim]\n"
        )

        if not creators_summary:
            console.print(
                "⚠️  No creator information found in this snapshot.", style="yellow"
            )
        ...
        if resources_without_creator > 0:
            console.print(
                f"  • Resources without creator info: {resources_without_creator:,}",
                style="yellow",
            )
        console.print()
        ...
        for creator_info in creators_summary:
            ...
            sorted_types = sorted(
                creator_info["resource_types"].items(), key=lambda x: x[1], reverse=True
            )[:3]
            types_display = ", ".join(
                f"{t.split('::')[-1]}({c})" for t, c in sorted_types
            )
        ...
        if detailed:
            ...
            for creator_info in creators_summary:
                ...
                console.print(
                    f"[bold]{creator_display}[/bold] ({creator_info['resource_count']} resources)"
                )
                ...
                for rtype, resources in sorted(resources_by_type.items()):
                    ...
                    for resource in resources[:5]:  # Show max 5 per type
                        ...
                        if resource.get("region"):
                            console.print(
                                f"      [dim]Region: {resource['region']}[/dim]"
                            )
                    if len(resources) > 5:
                        console.print(
                            f"    [dim]... and {len(resources) - 5} more[/dim]"
                        )
                    console.print()
...
def delta(
    ...
    inventory: Optional[str] = typer.Option(
        None, "--inventory", help="Inventory name (default: 'default')"
    ),
    ...
    show_details: bool = typer.Option(
        False, "--show-details", help="Show detailed resource information"
    ),
    show_diff: bool = typer.Option(
        False, "--show-diff", help="Show field-level configuration differences"
    ),
    export: Optional[str] = typer.Option(
        None, "--export", help="Export to file (JSON or CSV based on extension)"
    ),
    profile: Optional[str] = typer.Option(
        None, "--profile", "-p", help="AWS profile name"
    ),
    ...
    try:
        ...
        console.print(
            f"📦 Using inventory: [bold]{inventory_name}[/bold]", style="cyan"
        )
        ...
        if not active_inventory.snapshots:
            ...
            console.print(
                f"  Take a snapshot first: aws-snapshot create --inventory {inventory_name}",
                style="yellow",
            )
            raise typer.Exit(code=1)
        ...
        if snapshot:
            ...
            snapshot_name = active_inventory.active_snapshot.replace(
                ".yaml.gz", ""
            ).replace(".yaml", "")
            reference_snapshot = storage.load_snapshot(snapshot_name)

        console.print(
            f"🔍 Comparing to baseline: [bold]{reference_snapshot.name}[/bold]"
        )
        console.print(
            f"   Created: {reference_snapshot.created_at.strftime('%Y-%m-%d %H:%M:%S UTC')}\n"
        )
        ...
        if export:
            if export.endswith(".json"):
                ...
                console.print(
                    "✗ Unsupported export format. Use .json or .csv", style="bold red"
                )
                raise typer.Exit(code=1)
...
def cost(
    ...
    granularity: str = typer.Option(
        "MONTHLY", "--granularity", help="Cost granularity: DAILY or MONTHLY"
    ),
    show_services: bool = typer.Option(
        True, "--show-services/--no-services", help="Show service breakdown"
    ),
    export: Optional[str] = typer.Option(
        None, "--export", help="Export to file (JSON or CSV based on extension)"
    ),
    ...
    try:
        ...
        console.print(
            f"💰 Analyzing costs for snapshot: [bold]{reference_snapshot.name}[/bold]\n"
        )
        ...
        if start_date:
            try:
                ...
                console.print(
                    "✗ Invalid start date format. Use YYYY-MM-DD (UTC)",
                    style="bold red",
                )
                raise typer.Exit(code=1)
        ...
        if end_date:
            try:
                ...
                console.print(
                    "✗ Invalid end date format. Use YYYY-MM-DD (UTC)", style="bold red"
                )
                raise typer.Exit(code=1)
        ...
        if granularity not in ["DAILY", "MONTHLY"]:
            console.print(
                "✗ Invalid granularity. Use DAILY or MONTHLY", style="bold red"
            )
            raise typer.Exit(code=1)
        ...
        try:
            ...
            reporter.display(
                cost_report, show_services=show_services, has_deltas=has_deltas
            )
...
def security_scan(
    snapshot: Optional[str] = typer.Option(
        None, "--snapshot", "-s", help="Snapshot name to scan"
    ),
    inventory: Optional[str] = typer.Option(
        None, "--inventory", "-i", help="Inventory name (uses active snapshot)"
    ),
    storage_dir: Optional[str] = typer.Option(
        None, "--storage-dir", help="Snapshot storage directory"
    ),
    severity: Optional[str] = typer.Option(
        None, "--severity", help="Filter by severity: critical, high, medium, low"
    ),
    export: Optional[str] = typer.Option(
        None, "--export", help="Export findings to file"
    ),
    format: str = typer.Option(
        "json", "--format", "-f", help="Export format: json or csv"
    ),
    cis_only: bool = typer.Option(
        False, "--cis-only", help="Show only findings with CIS Benchmark mappings"
    ),
    profile: Optional[str] = typer.Option(
        None, "--profile", "-p", help="AWS profile name"
    ),
    ...
    try:
        ...
        if not snapshot and not inventory:
            console.print(
                "✗ Error: Must specify either --snapshot or --inventory",
                style="bold red",
            )
            raise typer.Exit(code=1)
        ...
        if inventory:
            ...
            snapshot_name = inv.active_snapshot.replace(".yaml.gz", "").replace(
                ".yaml", ""
            )
        ...
        if cis_only:
            findings_to_report = [
                f for f in result.findings if f.cis_control is not None
            ]
        ...
        if len(findings_to_report) == 0:
            ...
            console.print(
                f"[bold red]✗ Found {summary['total_findings']} security issue(s)[/bold red]\n"
            )
            console.print(
...
cleanup_app = typer.Typer(
    help="Delete resources - returns environment to baseline or removes unprotected resources"
)
...
def cleanup_preview(
    ...
    account_id: str = typer.Option(
        None, "--account-id", help="AWS account ID (auto-detected if not provided)"
    ),
    profile: Optional[str] = typer.Option(None, "--profile", help="AWS profile name"),
    ...
    regions: Optional[List[str]] = typer.Option(
        None, "--region", help="Filter by AWS regions"
    ),
    ...
    try:
        ...
        if protection_rules:
            console.print(
                f"[dim]Loaded {len(protection_rules)} protection rule(s)[/dim]"
            )
...
• Protected (skipped): {operation.skipped_count}
        ...
        console.print(
            Panel(
                summary_text.strip(),
                title="[bold]Preview Summary[/bold]",
                border_style="cyan",
            )
        )
        ...
        if operation.total_resources > operation.skipped_count:
            ...
            console.print(
                "[dim]Use 'awsinv cleanup execute' with --confirm to actually delete resources[/dim]\n"
            )
        else:
            console.print(
...
def cleanup_execute(
    ...
    profile: Optional[str] = typer.Option(None, "--profile", help="AWS profile name"),
    resource_types: Optional[List[str]] = typer.Option(
        None, "--type", help="Filter by resource types"
    ),
    regions: Optional[List[str]] = typer.Option(
        None, "--region", help="Filter by AWS regions"
    ),
    ...
    confirm: bool = typer.Option(
        False, "--confirm", help="Confirm deletion (REQUIRED for execution)"
    ),
    ...
    try:
        ...
        if not confirm:
            ...
            console.print(
                "[yellow]This is a safety measure to prevent accidental deletions[/yellow]"
            )
            console.print(
        ...
        console.print(
            f"• {preview_op.skipped_count} resource(s) will be skipped (protected)"
        )
        console.print(f"• Account: {account_id}")
        ...
        if preview_op.filters:
            if preview_op.filters.get("resource_types"):
                console.print(
                    f"• Types: {', '.join(preview_op.filters['resource_types'])}"
                )
        ...
        if failed_records:
            ...
            for record in failed_records:
                ...
                type_short = (
                    record.resource_type.split("::")[-1]
                    if "::" in record.resource_type
                    else record.resource_type
                )
                console.print(
                    f"  [red]✗[/red] [bold]{type_short}[/bold]: {record.resource_id}"
                )
                console.print(f"    [dim]Region:[/dim] {record.region}")
                console.print(f"    [dim]ARN:[/dim] {record.resource_arn}")
                console.print(
                    f"    [dim]Error:[/dim] [yellow]{record.error_message}[/yellow]"
                )
                console.print()
        ...
        for tag_pattern in exclude_tags:
            ...
            for tag_key, tag_value in resource_tags.items():
                if _matches_wildcard_pattern(
                    tag_key, key_pattern
                ) and _matches_wildcard_pattern(tag_value, value_pattern):
                    return (
                        True,
                        f"tag '{tag_key}={tag_value}' matches exclusion pattern '{tag_pattern}'",
                    )
...
def cleanup_purge(
    ...
    exclude_names: Optional[List[str]] = typer.Option(
        ...
        "--exclude-name",
        "-x",
        help="Exclude resources by name pattern (supports * and ? wildcards, can repeat)",
    ),
    exclude_tags: Optional[List[str]] = typer.Option(
        None,
        "--exclude-tag",
        help="Exclude resources by tag (format: key=value, supports wildcards, can repeat)",
    ),
    config_file: Optional[str] = typer.Option(
        None, "--config", help="Path to protection rules config file"
    ),
    ...
    created_by: Optional[str] = typer.Option(
        None,
        "--created-by",
        help="Only delete resources created by this user/role (substring match on creator ARN)",
    ),
    ...
    try:
        ...
        if (created_by or created_after or created_before) and not from_snapshot:
            ...
            console.print(
                "[dim]  awsinv snapshot enrich-creators <snapshot-name>[/dim]"
            )
            console.print(
        ...
        if created_after:
            try:
                created_after_dt = dt.fromisoformat(
                ...
                console.print(
                    "[yellow]Use ISO format: YYYY-MM-DD or YYYY-MM-DDTHH:MM:SS[/yellow]\n"
                )
                raise typer.Exit(code=1)
        if created_before:
            try:
                created_before_dt = dt.fromisoformat(
        ...
        if from_snapshot:
            ...
            console.print(
                f"[dim]Loaded {len(all_resources)} resources from snapshot[/dim]"
            )

            # Check if snapshot has creator info
            has_creator_info = any(
                r.tags and "_created_by" in r.tags for r in all_resources[:100]
            )  # Sample first 100
            if (created_by or created_after or created_before) and not has_creator_info:
                ...
                console.print(
                    "[dim]Run 'awsinv snapshot enrich-creators <snapshot>' first[/dim]\n"
                )
            ...
            for resource in all_resources:
                ...
                if resource_created_at and (created_after_dt or created_before_dt):
                    try:
                        resource_dt = dt.fromisoformat(
                            resource_created_at.replace("Z", "+00:00")
                        )
                    ...
                    created_after_dt or created_before_dt
                ) and not resource_created_at:
            ...
            console.print(
                f"[dim]After creator/date filters: {len(filtered_resources)} resources[/dim]"
            )
            ...
            target_regions = (
                regions if regions else ["us-east-1"]
        ...
        if from_snapshot:
            if resource_types:
                all_resources = [
                    r for r in all_resources if r.resource_type in resource_types
                ]
                console.print(
                    f"[dim]After type filter: {len(all_resources)} resources[/dim]"
                )
            if regions:
                all_resources = [r for r in all_resources if r.region in regions]
                console.print(
                    f"[dim]After region filter: {len(all_resources)} resources[/dim]"
                )
        ...
        console.print(
            f"  • Protected by rules (will keep): [green]{len(protected)}[/green]"
        )
        if excluded:
            console.print(
                f"  • Excluded by pattern (will keep): [cyan]{len(excluded)}[/cyan]"
            )
        ...
        if preview:
            ...
            if to_delete:
                ...
                for tier in sorted(resources_by_tier.keys()):
                    ...
                    for resource in tier_resources:
                        ...
                        console.print(
                            f"    [red]✗[/red] {type_short}: {resource.name} ({resource.region})"
                        )
            ...
            if excluded:
                ...
                for resource, reason in excluded:
                    console.print(
                        f"  [cyan]○[/cyan] {resource.resource_type}: {resource.name} - {reason}"
                    )
            ...
            if protected:
                ...
                for resource, reason in protected:
                    console.print(
                        f"  [green]✓[/green] {resource.resource_type}: {resource.name} - {reason}"
                    )
            ...
            for resource in to_delete:
                ...
                if success:
                    if was_skipped:
                        ...
                        skipped += 1
                        logger.info(
                            f"Skipped (already gone) {resource.resource_type}: {resource.name}"
                        )
                    else:
                        progress_display.mark_succeeded(resource)
                        succeeded += 1
                        logger.info(
                            f"Deleted {resource.resource_type}: {resource.name}"
                        )
                else:
                    ...
                    logger.warning(
                        f"Failed to delete {resource.resource_type}: {resource.name} - {error}"
                    )
        finally:
        ...
        status_color = (
            "green"
            if failed == 0
            else "yellow" if (succeeded > 0 or skipped > 0) else "red"
        )
        ...
        if snapshot:
            ...
            if match_where:
                ...
                match_clause = re.search(
                    r"(?i)\b(group\s+by|having|order\s+by|limit)\b", query
                )
                if match_clause:
...
def query_resources(
    type: Optional[str] = typer.Option(
        None, "--type", "-t", help="Filter by resource type (e.g., 's3:bucket', 'ec2')"
    ),
    ...
    arn: Optional[str] = typer.Option(
        None, "--arn", help="Filter by ARN pattern (supports wildcards)"
    ),
    snapshot: Optional[str] = typer.Option(
        None, "--snapshot", "-s", help="Limit to specific snapshot"
    ),
    limit: int = typer.Option(100, "--limit", "-l", help="Maximum results to return"),
    format: str = typer.Option(
...
def query_stats(
    snapshot: Optional[str] = typer.Option(
        None, "--snapshot", "-s", help="Specific snapshot (default: all)"
    ),
    group_by: str = typer.Option(
        "type", "--group-by", "-g", help="Group by: type, region, service, snapshot"
    ),
    format: str = typer.Option(
        "table", "--format", "-f", help="Output format: table, json"
    ),
...
def query_diff(
    ...
    try:
        ...
        if type:
            result["added"] = [
                r for r in result["added"] if type.lower() in r["resource_type"].lower()
            ]
            result["removed"] = [
                r
                for r in result["removed"]
                if type.lower() in r["resource_type"].lower()
            ]
            result["modified"] = [
                r
                for r in result["modified"]
                if type.lower() in r["resource_type"].lower()
            ]
            # Update counts
...
def group_list(
    ...
    try:
        ...
        if not groups:
            console.print(
                "[yellow]No groups found. Create one with 'awsinv group create'[/yellow]"
            )
            return
...
def group_compare(
    ...
    snapshot: str = typer.Option(
        ..., "--snapshot", "-s", help="Snapshot to compare against"
    ),
    format: str = typer.Option(
        "summary", "--format", "-f", help="Output format: summary, table, json"
    ),
    show_details: bool = typer.Option(
        False, "--details", "-d", help="Show individual resource details"
    ),
):
        ...
        if show_details or format == "table":
            ...
            if result["resources"]["missing"]:
                ...
                if len(result["resources"]["missing"]) > 25:
                    console.print(
                        f"[dim]... and {len(result['resources']['missing']) - 25} more[/dim]"
                    )
            ...
            if result["resources"]["extra"]:
                ...
                if len(result["resources"]["extra"]) > 25:
                    console.print(
                        f"[dim]... and {len(result['resources']['extra']) - 25} more[/dim]"
                    )
...
def group_add(
    ...
    try:
        ...
        if len(parts) != 2:
            console.print(
                "[red]Error: Resource must be specified as 'name:type' (e.g., 'my-bucket:s3:bucket')[/red]"
            )
            raise typer.Exit(code=1)
        ...
        if added > 0:
            console.print(
                f"[green]✓ Added '{resource_name}' ({resource_type}) to group '{name}'[/green]"
            )
        else:
...
def group_remove(
    name: str = typer.Argument(..., help="Group name"),
    resource: str = typer.Option(
        ..., "--resource", "-r", help="Resource to remove as 'name:type'"
    ),
):
    ...
    try:
        ...
        if removed:
            console.print(
                f"[green]✓ Removed '{resource_name}' ({resource_type}) from group '{name}'[/green]"
            )
        else:
...
def group_export(
    name: str = typer.Argument(..., help="Group name"),
    format: str = typer.Option(
        "yaml", "--format", "-f", help="Output format: yaml, csv, json"
    ),
            ...
            for member in group.members:
                writer.writerow(
                    [
                        member.resource_name,
                        member.resource_type,
                        member.original_arn or "",
                    ]
                )
            content = buffer.getvalue()
        else:  # yaml
            content = yaml.dump(
                group.to_dict(), default_flow_style=False, sort_keys=False
            )
...
def normalize(
    snapshot: str = typer.Option(
        ..., "--snapshot", "-s", help="Snapshot name to normalize"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Preview normalizations without saving"
    ),
    use_ai: bool = typer.Option(
        True, "--ai/--no-ai", help="Use AI for ambiguous names (default: enabled)"
    ),
):
    ...
    if not snapshot_obj or not snapshot_obj.resources:
        console.print(
            f"[yellow]⚠ Snapshot '{snapshot}' has no resources to normalize[/yellow]"
        )
        raise typer.Exit(code=0)
    ...
    if use_ai and not normalizer_config.is_ai_enabled:
        console.print(
            "[yellow]⚠ OPENAI_API_KEY not set - using rules-based normalization only[/yellow]"
        )
        use_ai = False
    ...
    console.print(
        f"[green]✓ Updated {updated_count} resources with normalized names[/green]"
    )
...
lambda_app = typer.Typer(
    help="Extract, view, and diff Lambda function code from snapshots"
)
...
def lambda_list(
    snapshot_name: Optional[str] = typer.Argument(
        None, help="Snapshot name (defaults to active)"
    ),
    show_all: bool = typer.Option(
        False, "--all", "-a", help="Show all lambdas including those without code"
    ),
):
    ...
    lambdas = [
        r for r in snapshot.resources if r.resource_type == "AWS::Lambda::Function"
    ]
    ...
    for fn in sorted(lambdas, key=lambda x: x.name or ""):
        ...
        if has_code:
            ...
            table.add_row(
                fn.name or "unnamed", runtime, "[dim]✗ (>10MB)[/dim]", "—", sha, "—"
            )
...
def lambda_extract(
    function_name: str = typer.Argument(
        ..., help="Lambda function name (or 'all' for all functions)"
    ),
    snapshot_name: Optional[str] = typer.Option(
        None, "--snapshot", "-s", help="Snapshot name (defaults to active)"
    ),
    output_dir: str = typer.Option(
        "./lambda_code", "--output", "-o", help="Output directory"
    ),
    flatten: bool = typer.Option(
        False, "--flatten", "-f", help="Extract all to single directory (no subdirs)"
    ),
):
    ...
    if function_name.lower() == "all":
        targets = [
            fn
            for fn in lambdas
            if fn.raw_config and fn.raw_config.get("_code", {}).get("code_stored")
        ]
    else:
        targets = [fn for fn in lambdas if fn.name == function_name]
        if not targets:
            console.print(
                f"[red]Lambda function '{function_name}' not found in snapshot.[/red]"
            )
            available = [fn.name for fn in lambdas[:10]]
            if available:
                console.print(
                    f"[dim]Available: {', '.join(available)}{'...' if len(lambdas) > 10 else ''}[/dim]"
                )
            raise typer.Exit(code=1)
    ...
    for fn in targets:
        ...
        try:
            ...
            console.print(
                f"[green]✓[/green] {fn.name}: {file_count} files → {extract_path}"
            )
            extracted_count += 1
...
def lambda_show(
    ...
    file_path: Optional[str] = typer.Option(
        None, "--file", "-f", help="Show specific file from package"
    ),
    list_files: bool = typer.Option(
        False, "--list", "-l", help="List files in package"
    ),
):
    ...
    fn = next(
        (
            ...
            if r.resource_type == "AWS::Lambda::Function" and r.name == function_name
        ),
        None,
    )
    ...
    with zipfile.ZipFile(BytesIO(zip_bytes)) as zf:
        ...
        if not target_file or target_file not in files:
            ...
            console.print(
                f"[dim]Available: {', '.join(files[:10])}{'...' if len(files) > 10 else ''}[/dim]"
            )
            raise typer.Exit(1)
...
def lambda_diff(
    ...
    file_path: Optional[str] = typer.Option(
        None, "--file", "-f", help="Diff specific file"
    ),
):
    ...
    if not bytes1 or not bytes2:
        console.print(
            "[yellow]Cannot show diff - one or both snapshots missing code[/yellow]"
        )
        raise typer.Exit(0)
...
def lambda_fetch(
    ...
    force: bool = typer.Option(
        False, "--force", help="Re-fetch code even if already stored"
    ),
    profile: Optional[str] = typer.Option(
        None, "--profile", "-p", help="AWS profile name"
    ),
    no_ssl_verify: bool = typer.Option(
        False,
        "--no-ssl-verify",
        help="Disable SSL certificate verification for S3 downloads",
    ),
):
        ...
        for fn in lambdas:
            ...
            try:
                ...
                try:
                    ...
                    console.print(
                        f"[yellow]⚠ {fn.name}: Function not found in AWS ({e})[/yellow]"
                    )
                    skip_count += 1
                ...
                if not code_location:
                    console.print(
                        f"[yellow]⚠ {fn.name}: No code location available[/yellow]"
                    )
                    skip_count += 1
                    progress.advance(task)
                    continue
                ...
                response = requests.get(
                    code_location, timeout=120, verify=not no_ssl_verify
                )
                ...
                if max_size_bytes == -1 or code_size <= max_size_bytes:
                    # Store inline
                    code_data["code_base64"] = base64.b64encode(code_bytes).decode(
                        "utf-8"
                    )
                    ...
                    # Store externally
                    file_path, _ = code_storage.store_code(
                        snapshot_name, fn.name, code_bytes
                    )
                    code_data["code_stored"] = True
                ...
                size_str = (
                    f"{code_size / (1024 * 1024):.1f}MB"
                    if code_size >= 1024 * 1024
                    else f"{code_size / 1024:.1f}KB"
                )
                console.print(
                    f"[green]✓[/green] {fn.name}: {size_str} ({storage_type})"
                )
                success_count += 1
...
def copilot_install(
    ...
    if json_output:
        output = {
            ...
            "backed_up": [
                [str(orig), str(backup)] for orig, backup in result.backed_up
            ],
        ...
        if result.success:
            console.print(
                f"[green]✓ Installed {len(result.installed)} files to {target_path}/.github/[/green]"
            )
...
def generate(
    format: str = typer.Argument(
        ..., help="Output format: terraform, cdk-typescript, cdk-python"
    ),
    snapshot_name: Optional[str] = typer.Argument(
        None, help="Name of snapshot to generate from"
    ),
    output: str = typer.Option(
        "./terraform", "--output", "-o", help="Output directory"
    ),
    ...
    model_id: Optional[str] = typer.Option(
        ...
        "--model-id",
        "-m",
        help="Bedrock model ID (default: from AWSINV_BEDROCK_MODEL_ID)",
    ),
    region: Optional[str] = typer.Option(
        None,
        "--region",
    ...
    Formats:
        ...
        cdk-typescript - AWS CDK TypeScript project
        cdk-python     - AWS CDK Python project
    ...
    if format not in ["terraform", "cdk-typescript", "cdk-python"]:
        console.print(
            f"[red]Error:[/red] Unknown format '{format}'. Use: terraform, cdk-typescript, cdk-python"
        )
        raise typer.Exit(1)
    ...
    try:
        from ..generate.cdk_generator import generate_cdk
    ...
    node_to_step = {
        ...
        # CDK validation (map to terraform steps for now - progress display works the same)
        "npm_build": WorkflowStep.TERRAFORM_INIT,  # Similar purpose: install deps
        "cdk_synth": WorkflowStep.TERRAFORM_VALIDATE,  # Similar purpose: validate code
    }
...
def compare(
    snapshot_name: Optional[str] = typer.Argument(
        None, help="Name of snapshot to compare against"
    ),
    iac_dir: str = typer.Option(
        "./terraform", "--iac-dir", "-d", help="Directory containing IaC files"
    ),
    from_file: Optional[str] = typer.Option(
        None,
        "--from-file",
    ...
    if output_json:
        ...
        coverage_color = (
            ...
            else "yellow" if result.coverage_percentage >= 70 else "red"
        )
        console.print(
            f"  [bold]Coverage:[/bold] [{coverage_color}]{result.coverage_percentage:.1f}%[/{coverage_color}]"
        )
        console.print(
            f"  [bold]Resources:[/bold] {result.represented_count}/{result.total_resources} represented"
        )
        ...
        if result.missing_count > 0:
            ...
            for missing in result.missing_resources[:10]:  # Show first 10
                console.print(
                    f"    - {missing.get('type', 'unknown')}: {missing.get('name', 'unnamed')}"
                )
        ...
        if result.issues:
            ...
            for issue in result.issues[:5]:  # Show first 5
                ...
                console.print(
                    f"    [{color}]{severity}[/{color}]: {issue.get('description', '')}"
                )
